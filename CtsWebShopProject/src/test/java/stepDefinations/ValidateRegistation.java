...
import testResources.ExcelUtilities;

@RunWith(Cucumber.class)
public class ValidateRegistation extends Base {
	final static Logger log = LogManager.getLogger(ValidateRegistation.class.getName());
	...
	@When("^user enters \"([^\"]*)\" Navigate to Home page$")
	public void user_enters_Navigate_to_Home_page(String arg1) throws Throwable {
	...
	@When("^click on Register in home page to land on secure Registration page$")
	...
	public void user_enters_registration_details_and_clicks_on_Register() throws Throwable {
		...
		data = new String[5];
		for (int i = 1; i <= 5; i++) {
			data[j] = xl.readXL("src\\test\\Resource\\testResources\\webshops.xlsx", i, 1);
			System.out.println(data[j]);
			j++;
		}
		log.debug("entering the registation details");
		rp.RegisterDetails(data);
